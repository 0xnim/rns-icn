...
async def run_integration_test():
    ...
    print("   Server created, calling start()...")
    await server.start()
    ...
    print(f"\n      CS entries: {len(server.forwarder.cs)}")
    ...
    if manifest_data:
        ...
        print(f"      ✗ Manifest not found in CS!")

    # Give mesh time to propagate
    print("\n[5/5] Testing client fetch...")
    await asyncio.sleep(3)

    # Test ICNClient fetch - client MUST be in same process to reuse RNS
    print("      Starting client fetch test (same process)...")
